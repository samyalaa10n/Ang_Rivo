...
export class SpeciaPriceComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.Columns.push(new Column('ID_ITEM', "Item", "comboBox"))
    this.Columns[this.Columns.length - 1].columnComboBoxOptionLabel = "NAME";
    this.Columns[this.Columns.length - 1].columnComboBoxOptionValue = "ID";
    ...
    this.Columns.push(new Column('PRICE', "New Price", "numberWithFraction"))
  ...
  GridConfig(grid: DataGridComponent) {
    ...
    grid.Copy = () => {
      this._tools.Toaster.showInfo("Cannot copy from this list")
    }
    grid.Pest = async () => {
      var data = JSON.parse(await navigator.clipboard.readText()) as Array<any>;
      let result = await this._tools.DecisionMaker.Show("Select Customer", this.Customers.map(x => x.NAME))
      if (result) {
        let Customer_Id = this.Customers.find(x => x.NAME == result).ID;
        if (Customer_Id) {
          let Season = await this._tools.DecisionMaker.Show("Select Season", this.Seasons.map(x => x.NAME))
          if (Season) {
